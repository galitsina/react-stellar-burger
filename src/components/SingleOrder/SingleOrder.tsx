...
const SingleOrder: FC = () => {
  ...
  const uniqIngredientObj: { [key: string]: number } =
    ingredientsInOrder.reduce(
    ...
    return (
      <div className={singleOrderStyles.ingredient} key={i}>
        <div className={singleOrderStyles.image_border}>
          <img
            className={singleOrderStyles.image_icon}
            src={ingredientObj?.image}
            alt={ingredientObj?.name}
          />
        </div>
        <p
          className={`${singleOrderStyles.ingredient_name} text text_type_main-default`}
        >
          {ingredientObj?.name}
        </p>
        <div className={singleOrderStyles.price}>
          <p className="text text_type_digits-default ">
            {value} x {ingredientObj?.price}
          </p>
    ...
    <>
      {items.length && Boolean(singleOrderDetails) && (
        <div className={`${singleOrderStyles.container} mb-15`}>
          <p
            className={`${singleOrderStyles.order_number} text text_type_digits-default mb-10`}
          >
            #{number}
          </p>
          ...
          <div
            className={`${singleOrderStyles.order_list} custom-scroll mb-10 pr-8`}
          >
            {ingredientList}
          </div>
